...
export default function BookmarksTest() {
    ...
    const fetchAllPosts = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (userId) {
            await checkBookmarkStatusesForPosts(posts);
          }
        }
      } catch (error) {
  ...
  useEffect(() => {
    const fetchBookmarkedPosts = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Fetch full post data for each bookmark to get images and other details
          const bookmarkedPostsData = await Promise.all(
            bookmarksArray.map(async (bookmark) => {
              ...
              // Fallback to bookmark data if post fetch fails
              return {
                bookmark,
                post: {
    ...
    if (activeTab === 'bookmarked' && userId) {
    ...
    try {
      if (!isBookmarked) {
        ...
        if (response.ok || response.status === 409) {
          ...
          // Refresh bookmarked posts if on that tab
  ...
  // Render a post card
  const renderPostCard = (post: Post, isBookmarked: boolean) => {
    return (
      <PostCard 
        key={post._id} 
        post={post} 
  ...
  // Render a bookmarked post card
  const renderBookmarkedPostCard = (bookmarkedPostData: BookmarkedPostData) => {
    const { bookmark, post } = bookmarkedPostData;
    
    if (!post) {
      // Fallback if post data wasn't fetched
      ...
      return (
        <div key={bookmark.bookmark_id}>
          {renderPostCard(fallbackPost, true)}
        </div>
      );
    }
  ...
  return (
    <div className="w-full max-w-4xl mx-auto rounded-lg bg-white p-8 shadow-lg">
      ...
      {activeTab === 'all' && (
        <div>
          {allPosts.map((post) => renderPostCard(post, bookmarkStatuses[post._id] || false))}
        </div>
      )}

      {activeTab === 'bookmarked' && (
        <div>
          {bookmarkedPosts.map((bookmark) => renderBookmarkedPostCard(bookmark))}
        </div>
      )}
    </div>
  );
